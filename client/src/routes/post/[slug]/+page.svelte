...
{:else if notFound}
	<div class="flex flex-col items-center justify-center gap-4 pt-16">
		...
		<a
			href={resolve('/')}
			class="rounded-sm bg-linear-to-r from-pink to-mauve px-4 py-2 text-crust hover:opacity-90"
		>
			Go to Home
		</a>
	</div>
		...
		<div class="order-2 px-4 md:order-1 md:w-80 md:flex-none">
			<div class="flex flex-row items-center justify-center gap-2">
				<form onsubmit={submit} class="flex w-full flex-col gap-2">
					...
					<button
						type="submit"
						class="mb-2 w-full rounded-full bg-linear-to-r from-pink to-mauve px-4 py-2 text-crust hover:cursor-pointer hover:opacity-90"
					>
						Search
					</button>
				</form>
			...
			<div class="flex flex-row items-center justify-between gap-2">
				<button
					type="button"
					class="flex w-full items-center justify-center rounded-sm bg-mantle px-4 py-2 text-center text-text transition-colors hover:cursor-pointer hover:bg-surface1 disabled:cursor-not-allowed disabled:opacity-50"
			...
			<a href={`${resolve('/')}data/media/${slug}.${mediaExt}`} download>
				<button
					type="button"
					class="mt-2 flex w-full items-center justify-center rounded-full bg-linear-to-r from-sapphire to-blue px-4 py-2 text-center text-black transition-colors hover:cursor-pointer hover:bg-surface1"
				>
			...
			{#if canDeletePost}
				<button
					type="button"
					disabled={isDeleting}
					class="mt-2 flex w-full items-center justify-center rounded-full border bg-linear-to-r from-maroon to-red px-4 py-2 text-center text-black transition-colors hover:cursor-pointer hover:bg-red/20 disabled:cursor-not-allowed disabled:opacity-50"
					onclick={deletePost}
				>
					<div class="flex w-full flex-row justify-between gap-2">
			...
			<div class="mt-2 grid w-full grid-cols-2 gap-2">
				<div class="rounded-sm bg-mantle px-3 py-2 text-center">
			...
			<div class="mt-2 rounded-sm bg-mantle px-3 py-2 text-sm">
			...
			{/if}
			{#if tagEditError}
				<p class="mt-2 rounded-sm bg-red/20 px-4 py-2 text-red">{tagEditError}</p>
			{/if}
				...
				<div class="mt-3 flex flex-col gap-2">
					...
					<div class="mt-1 flex gap-2">
						<button
							...
							class="flex-1 rounded-full bg-linear-to-r from-pink to-mauve px-4 py-2 font-semibold text-crust hover:cursor-pointer hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
							onclick={saveTags}
						>
						...
						<button
							type="button"
							class="flex-1 rounded-full bg-linear-to-r from-red to-maroon px-4 py-2 text-black transition-colors hover:cursor-pointer hover:bg-surface1"
			...
			{:else}
				{#if canEditPostTags}
					<button
						type="button"
						class="mt-3 w-full rounded-full bg-linear-to-r from-green to-teal px-4 py-2 text-black transition-colors hover:cursor-pointer hover:bg-surface1"
